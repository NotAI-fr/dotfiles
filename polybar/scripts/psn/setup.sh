...
repair_permissions() {
    ...
    if [[ -e "${APP_DIR}/npsso" ]]; then
        chmod 600 "${APP_DIR}/npsso"
    fi
    if [[ -e "${APP_DIR}/auth_tokens.json" ]]; then
        chmod 600 "${APP_DIR}/auth_tokens.json"
    fi

    # Always return success when required files were repaired. Optional files
    # being absent is normal during a clean setup.
    return 0
}

validate_auth_cache_file() {
    local cache_file="${APP_DIR}/auth_tokens.json"
    [[ -e "${cache_file}" ]] || return 0

    if "${VENV_DIR}/bin/python" - "${cache_file}" <<'PY'
import json
import sys
from pathlib import Path
...
PY
    ...
    local invalid="${cache_file}.invalid-$(timestamp)"
    mv "${cache_file}" "${invalid}"
    warn "Malformed reusable token cache moved to: ${invalid}"
}
...
install_or_update() {
    ...
    ensure_venv
    repair_permissions
    ok "Runtime files and permissions installed"
    test_or_renew_token
    full_refresh
    say
...
repair() {
    ...
    ensure_venv
    repair_permissions
    ok "Runtime files and permissions repaired"
    test_or_renew_token
    full_refresh
    say
    "${APP_DIR}/psn-friends" doctor
